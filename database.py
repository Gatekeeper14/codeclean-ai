...
def init_db():
    ...
    try:
        ...
        """)
        conn.commit()
        print("DATABASE READY — CodeClean v1.0")
    finally:
        release_db(conn)

def register_user(tid, username):
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("SELECT telegram_id FROM users WHERE telegram_id=%s", (tid,))
        if cur.fetchone(): return False
        from config import FREE_CLEANS
        cur.execute("INSERT INTO users (telegram_id, username, credits) VALUES (%s,%s,%s) ON CONFLICT DO NOTHING",
...
def get_user(tid):
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("SELECT telegram_id, username, credits, free_cleans_used, total_cleans, is_pro FROM users WHERE telegram_id=%s", (tid,))
        return cur.fetchone()
    finally:
        release_db(conn)
...
def deduct_credit(tid):
    ...
    try:
        ...
        cur.execute("UPDATE users SET credits=credits-1, total_cleans=total_cleans+1 WHERE telegram_id=%s", (tid,))
        conn.commit(); return True
    finally:
        release_db(conn)
...
def log_clean(tid, file_type, input_method, issues_fixed, fix_report):
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO clean_history (telegram_id, file_type, input_method, issues_fixed, fix_report) VALUES (%s,%s,%s,%s,%s)",
...
def get_history(tid, limit=10):
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("SELECT file_type, input_method, issues_fixed, cleaned_at FROM clean_history WHERE telegram_id=%s ORDER BY cleaned_at DESC LIMIT %s", (tid, limit))
        return cur.fetchall()
    finally:
        release_db(conn)
